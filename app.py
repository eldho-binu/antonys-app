...
MONGO_URI = os.getenv('MONGO_URI', 'mongodb+srv://st_antonys:antony%40123@stantonys.uwsiqgw.mongodb.net/')
...
def connect_to_mongodb():
    ...
    try:
        ...
        db = client['St_antonys_ayurvedics']
...
def initialize_admin():
    if admins_collection is None:
        print("⚠️  Cannot initialize admin - no database connection")
        return
...
def find_patient_by_regno(regno):
    ...
    try:
        ...
        patient = patients_collection.find_one({'regno': search_regno})
        if patient:
            return patient
...
def is_regno_exists(regno):
    ...
    try:
        ...
        existing = patients_collection.find_one({'regno': search_regno})
...
if __name__ == '__main__':
    print(f"🚀 Starting St Antonys Ayurvedic API server...")
